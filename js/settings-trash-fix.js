...
(()=>{
  ...
  function openSettings(){
    try{ $('drawerOverlay')?.classList.remove('open'); document.querySelectorAll('.screen').forEach(x=>x.hidden=x.id!=='settingsScreen'); window.MGState&&(window.MGState.screen='settingsScreen'); window.MGAppCore?.renderSettings?.(); const tab=localStorage.getItem(TAB)||'catalog'; setTab(tab); window.scrollTo(0,0); }
  ...
  function renderTrash(){
    ...
    el.innerHTML=a.map(e=>{
      ...
      return `<article class="estimate-card trash-estimate-card"><div class="estimate-main"><div class="estimate-badge">MG</div><div class="estimate-info"><b>${esc(e.number||'Смета')}</b><span>${esc(e.date||'')} · ${money(e.total)} MDL</span><small>${esc(e.client||'Без клиента')} · ${esc(e.phone||'Без телефона')}</small><small>${esc(e.city||'')} ${e.address?'· '+esc(e.address):''}</small><small>${esc(dirs)} · ${rows.length} позиций</small><small>Удалена: ${formatDate(e._deletedAt)}</small></div></div><div class="estimate-actions"><button type="button" class="btn secondary" data-trash-view="${esc(String(e.id))}">Посмотреть</button><button type="button" class="btn primary" data-trash-restore="${esc(String(e.id))}">Восстановить</button><button type="button" class="btn danger" data-trash-delete="${esc(String(e.id))}">Удалить навсегда</button></div></article>`;
    }).join('');
  }
  ...
  function hardDelete(id){
    ...
    writeTrash(readTrash().filter(x=>String(x.id)!==String(id)));renderTrash();$('trashDetailModal')?.classList.remove('open');window.__mgToast?.('Смета удалена навсегда');
  }
  const handler=e=>{
